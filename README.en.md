...
- 🌍 **Internationalization**: Multi-language interface support. Since v1.2.0, the translation **domain strings of the 5 core business modules (`compile_engine` / `detection` / `compile_report` / `cli_workflow` / `file_ops`)** are in **1:1 strict alignment** with `set_language(...)` parameters and locale `.pot` filenames. All newly added user-facing strings are **100% wrapped with `_("…") + %(placeholder)s`**. The `.pot` translation templates are **automatically extracted via the system `xgettext`** tool (not pybabel, not hand-written), ensuring every msgid carries the source file:line location above it. Per user requirement Q2, **no `.mo` binary is generated** to avoid drift with the latest .pot
...
- 🚫 **Zero-compatibility promise + dead code cleanup**: Since v1.2.0 **zero compatibility layers are retained** (the old `pytexmk.run` entry / DeprecationWarning / try-import fallbacks are physically removed). A dual pass of `ruff F401/F841` static scan + dead-comment removal + duplicate file-read consolidation is run on the 5 core modules to keep architectural entropy under control
