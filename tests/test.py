    ...
    for i in range(len(test_files)):
        try:
            ...
            result = subprocess.run(
                run_command + [test_files[i]], cwd=destination_folder
            )
            time_end = datetime.datetime.now()
            time_run = round((time_end - time_start).total_seconds(), 4)
            if result.returncode == 0:
                print_output.append(
                    [test_file_type[i], time_run, "[green]通过"]
                )  # 绿色
            else:
                print_output.append(
                    [test_file_type[i], time_run, "[red]未通过"]
                )  # 红色
        except Exception:
    ...
    for i in range(len(command)):
        try:
            if command[i] == "-nq":
                time_start = datetime.datetime.now()
                result = subprocess.run(
                    run_command + ["-nq", "main"], cwd=destination_folder
                )
                time_end = datetime.datetime.now()
                time_run = round((time_end - time_start).total_seconds(), 4)
                if result.returncode == 0:
                ...
                result = subprocess.run(
                    run_command + [command[i]], cwd=destination_folder
                )
                time_end = datetime.datetime.now()
                time_run = round((time_end - time_start).total_seconds(), 4)
                if result.returncode == 0:
...
def print_table(data):
    ...
    table = Table(
        show_header=True, header_style="bold magenta", title="PyTeXMK 测试统计表"
    )
