...
# 注意：winget 发布不提供本地/Make 入口。
# 发布完全在 GitHub Action（Release.yml 的 publish-to-winget job）中，
# 由 tools/winget/publish.py（基于官方 wingetcreate）自动执行；
# 版本号自动取 config.__version__（src/pytexmk/version.py）。
