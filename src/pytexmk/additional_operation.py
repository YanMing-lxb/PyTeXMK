...
LastEditTime : 2024-06-29 14:09:29 +0800
...
def remove_result_in_root(project_name):
...
# 定义移动生成文件
# --------------------------------------------------------------------------------
def move_result(project_name, outdir):
    result_files = [f"{project_name}{ext}" for ext in [".pdf", ".synctex.gz"]]
...
# 定义清理所有 pdf 文件
# --------------------------------------------------------------------------------
def clean_pdf(root_dir, excluded_folder, project_name):
    pdf_files = []
...
def check_project_name(project_name):
    base_name, file_extension = os.path.splitext(
        os.path.basename(project_name))  # 去掉路径，提取文件名和后缀
    if file_extension == '.tex':  # 判断后缀是否是 .tex
        project_name_return = base_name
    elif '.' not in project_name:  # 判断输入 project_name 中没有 后缀
        if '/' in project_name or '\\' in project_name:  # 判断是否是没有后缀的路径
            project_name_return = None
            print("错误：文件路径无效！")
        else:
            project_name_return = project_name
    else:
        project_name_return = None
        print("提示：文件后缀不是.tex")
                ...
                with open(file_path, 'r', encoding='utf-8') as file:  # 打开文件
                    for _ in range(200):  # 遍历文件的前200行
                        ...
                        if ("\documentclass" in line or r"\begin{document}" in line):
                            ...
                            project_name = check_project_name(file_path)
