...
def get_main_file(args_document, main_file_in_root, all_magic_comments):
    ...
    if args_document: # 当前目录下存在 tex 文件, 且命令行参数中指定了主文件
        ...
        if len(all_magic_comments['root']) == 1: # 当前目录下存在多个主文件, 且只有一个存在 % TEX root 魔法注释
            logger.info(f"魔法注释 % !TEX root 只存在于: {all_magic_comments['root'][0][0]}.tex")
            ...
            if f"{all_magic_comments['root'][0][0]}" == f"{check_file}": # 如果 magic comments 中指定的 root 文件名与当前文件名相同
                ...
                logger.warning(f"魔法注释 % !TEX root 指定的文件名与当前文件名不同, 无法确定主文件:  [bold red]{check_file}.tex[/bold red], [bold green]{all_magic_comments['root'][0][0]}.tex[/bold green] ")
