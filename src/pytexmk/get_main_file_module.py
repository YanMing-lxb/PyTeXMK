...
LastEditTime : 2024-08-09 15:18:21 +0800
...
def get_main_file(args_document, main_file_in_root, all_magic_comments):
    ...
    if args_document: # 当前目录下存在 tex 文件, 且命令行参数中指定了主文件
        ...
        print(_("通过命令行命令指定待编译主文件为: ") + f"[bold cyan]{project_name}[/bold cyan]")
    elif len(main_file_in_root) == 1: # 如果当前根目录下存在且只有一个主文件
        project_name = main_file_in_root[0] # 使用该文件作为待编译主文件
        print(_("通过根目录下唯一主文件指定待编译主文件为: ") + f"[bold cyan]{project_name}.tex[/bold cyan]")
        ...
        if len(all_magic_comments['root']) == 1: # 当前目录下存在多个主文件, 且只有一个存在 % TEX root 魔法注释
            ...
            if f"{all_magic_comments['root'][0][0]}" == f"{check_file}": # 如果 magic comments 中指定的 root 文件名与当前文件名相同
                ...
                print(_("通过魔法注释 % !TEX root 指定待编译主文件为: ") + f"[bold cyan]{project_name}.tex[/bold cyan]")
        ...
        for file in main_file_in_root:
            if file == "main": # 如果存在 main.tex 文件
                ...
                print(_("通过默认文件名 \"main.tex\" 指定待编译主文件为: ") + f"[bold cyan]{project_name}.tex[/bold cyan]")
    ...
    if not project_name: # 如果当前根目录下不存在主文件且 -d 参数未指定
        ...
        logger.warning(_("当前根目录是: ") + current_path)
        exit_pytexmk()
