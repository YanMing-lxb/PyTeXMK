...
_ = set_language("timing")

total_len = 78

# 不再依赖中文翻译子串("编译"/"宏包"/"运行"), 避免本地化后筛选失效.
_LATEX_ENGINE_KEYWORDS = (
    "PdfLaTeX", "LuaLaTeX", "XeLaTeX", "DVIPDFMX",
    "biber", "bibtex", "makeindex", "latexdiff",
)
...
def time_print(start_time, runtime_dict):
    ...
    time_LaTeX_list = [
        ...
        if any(keyword in key for keyword in _LATEX_ENGINE_KEYWORDS)
    ]
    ...
    console.print(_("运行 LaTeX 程序数目: ") + f"{number_programmes_run}")
