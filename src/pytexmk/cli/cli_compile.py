...
def handle_clean_any(args, state: WorkflowState) -> bool:
    ...
    runtime_remove_aux_matched_auxdir, _ret = time_count(state.mro.remove_matched_files, state.aux_regex_files, ".")
    state.runtime_dict[_("清除所有的辅助文件")] = runtime_remove_aux_matched_auxdir

    if args.Clean_any:
        runtime_remove_out_outdir, _ret = time_count(state.mro.remove_specific_files, state.out_files, state.outdir)
...
def run(args, state: WorkflowState) -> None:
    ...
    if args.clean or args.Clean:
        ...
        runtime_pdf_repair, _ret = time_count(state.pfo.pdf_repair, state.project_name, ".", state.outdir)
...
def _clean(args, state: WorkflowState) -> None:
    ...
    runtime_remove_aux_auxdir, _ret = time_count(state.mro.remove_specific_files, state.aux_files, state.auxdir)
    state.runtime_dict[_("清除文件夹内辅助文件")] = runtime_remove_aux_auxdir
    runtime_remove_aux_root, _ret = time_count(state.mro.remove_specific_files, state.aux_files, ".")
...
def _compile(args, state: WorkflowState) -> None:
    ...
    RUN(
        state.runtime_dict, state.project_name, state.compiled_program, state.out_files, state.aux_files,
    ...
    runtime_move_out_outdir, _ret = time_count(state.mro.move_specific_files, state.out_files, ".", state.outdir)
    state.runtime_dict[_("结果文件->输出目录")] = runtime_move_out_outdir

    print("[yellow]" + _("移动辅助文件到辅助目录...") + "[/yellow]")
    runtime_move_aux_auxdir, _ret = time_count(state.mro.move_specific_files, state.aux_files, ".", state.auxdir)
    state.runtime_dict[_("辅助文件->辅助目录")] = runtime_move_aux_auxdir

    pytexlogs.run_log_pipeline(
        state.project_name, state.auxdir, root_file=state.project_name,
