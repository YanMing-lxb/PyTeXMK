...
class UpdateChecker:
    ...
    def _load_cached_version(self):
        ...
        try:
            ...
            # 使用timedelta来转换秒数
            cache_time_remaining = round(
                self.cache_time - (time.time() - cache_path.stat().st_mtime), 4
            )  # 计算缓存剩余时间
    ...
    def _get_latest_version(
        ...
        try:
            ...
            if e.code == 403 and "X-RateLimit-Remaining" in e.headers:
                reset_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(e.headers["X-RateLimit-Reset"])))
    ...
    def check_for_updates(self):
        ...
        if current_version < latest_version:
            ...
            print(
                _("请运行 [bold green]'pip install --upgrade %(args)s'[/bold green] 进行更新") % {"args": script_name}
            )
        else:
